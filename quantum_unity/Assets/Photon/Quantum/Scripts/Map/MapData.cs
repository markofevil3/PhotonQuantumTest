  ...
  void DrawGizmos(bool selected) {
    ...
    if (Asset) {
      ...
      if (QuantumEditorSettings.Instance.DrawStaticMeshTriangles || QuantumEditorSettings.Instance.DrawStaticMeshNormals) {
        if (Asset.Settings.StaticColliders3DTriangles == null && string.IsNullOrEmpty(Asset.Settings.StaticColliders3DTrianglesBinaryFile) == false) {
          FileLoader.Init(new UnityFileLoader(QuantumEditorSettings.Instance.DatabasePath));

          // loading primitive triangles from binary file
          var buffer = FileLoader.Load(Asset.Settings.StaticColliders3DTrianglesBinaryFile);
          if (buffer.Length > 0) {
            var stream = new ByteStream(buffer);
            Asset.Settings.SerializeStaticColliderTriangles(stream, false);
          }
          else {
            Debug.LogWarning("Map cannot load triangles data file from " + Asset.Settings.StaticColliders3DTrianglesBinaryFile);
          }
