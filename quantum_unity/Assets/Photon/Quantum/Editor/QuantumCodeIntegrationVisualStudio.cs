...
namespace Quantum.Editor {
  internal class QuantumCodeIntegrationVisualStudio : AssetPostprocessor {
    ...
    private static void Initialize() {
      EditorApplication.delayCall += () => {
        ...
        try {
          ...
          Debug.LogWarning($"Failed to set up Quantum Code Visual Studio Integration: {ex}");
        }
      };
    }
#else
    ...
    private static string PatchSolution(string solutionPath, string solutionContent) {
      ...
      var altered = PatchSolution(solutionContent, quantumSolutionPath,
        new string[] { }
      );
...
<configuration>
  <config>
    ...
    private static string PatchSolution(string solutionContent, string quantumSolutionPath, string[] quantumProjectsToIgnore) {
